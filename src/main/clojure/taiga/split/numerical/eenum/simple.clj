...
(ns ^{:author "John Alan McDonald, Kristina Lisa Klinkner" :date "2017-01-03"
      :doc "Greedy split on Enum-valued attributes." }
    
    taiga.split.numerical.eenum.simple
...
(defn- sorted-xy [score options]
  (let [data (:data options)
        ...
        ^java.util.Map groups (z/group-enums-by-not-nil-random-access x data)
        n (z/count groups)
        _ (assert (< 0 n) 
            ...
            (do
              (if (or (>= 0 imin) (>= cmin Double/POSITIVE_INFINITY))
                ...
                (let [cats (if (> nleft (- n nleft))
                             (utils/enum-set xs imin nx)
                             (utils/enum-set xs 0 imin))]
