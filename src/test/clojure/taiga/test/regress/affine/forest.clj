...
(ns ^{:author "wahpenayo at gmail dot com"
      :date "2018-02-01"
      :doc "Affine functional regression forest example." }
    
    taiga.test.regress.affine.forest
...
;; mvn -Dtest=taiga.test.regress.pyramid.mean clojure:test > tests.txt
;;------------------------------------------------------------------------------
(def nss (str *ns*))
(test/deftest affine-mean-regression
  (z/seconds 
    nss
    ...
    (let [options (defs/options (record/make-pyramid-function 10.0))
          ...
          _ (println "train:" )
          train-summary (defs/print-residual-summary y yhat (:data options))
          _ (println "test:" )
          test-summary (defs/print-residual-summary y yhat (:test-data options))]
