...
;; mvn -Dtest=taiga.test.regress.engel.l2 clojure:test > engel-l2.txt
;;----------------------------------------------------------------
(def nss (str *ns*))
;;----------------------------------------------------------------
(defn- l2-test [fit]
...
#_(test/deftest forest
    ...
    (z/seconds 
      nss
      (let [af (engel/make-affine-functional 10.0)
            ymean (fn ymean ^double [^Record datum]
                    (.invokePrim af
                      (engel/embedding engel/xbindings datum)))
            ...
            y (:ground-truth engel/attributes)
            yhat (fn yhat ^double [datum] 
                   (model engel/xbindings datum))
            _ (println "train:" )
            train-summary (defs/print-residual-summary 
                            y yhat (:data options))
            _ (println "test:" )
