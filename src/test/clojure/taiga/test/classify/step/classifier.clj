...
(ns ^{:author "John Alan McDonald" :date "2016-12-22"
    ...
    taiga.test.classify.step.classifier
...
;; mvn -Dtest=taiga.test.classify.step.classifier clojure:test > tests.txt
;;------------------------------------------------------------------------------
(def nss (str *ns*))
(test/deftest step-classifier
  ...
  (let [options (defs/options (record/make-diagonal-step-function 1.0) )
        ...
        c (:ground-truth record/attributes)
        chat (fn yhat ^double [datum] (forest predictors datum))
        _ (println "train:" )
        train-confusion (defs/print-confusion c chat (:data options))
        _ (println "test:" )
        train-confusion-rate (defs/confusion-rate train-confusion)
        _(println "train-confusion-rate" train-confusion-rate)
          test-confusion (defs/print-confusion c chat (:test-data options))
          test-confusion-rate (defs/confusion-rate test-confusion)]
    (test/is (== (z/count (:data options)) (reduce + train-confusion)))
    (test/is (== (float train-confusion-rate) (float 0.0)))
    ...
    (test/is (= [16620 0 0 16148] train-confusion))
    (test/is (== (z/count (:test-data options)) (reduce + test-confusion)))
    (println "test-confusion-rate" test-confusion-rate)
    (test/is (== (float test-confusion-rate) (float 3.662109375E-4)))
    (test/is (= [16422 6 6 16334] test-confusion))))
;------------------------------------------------------------------------------