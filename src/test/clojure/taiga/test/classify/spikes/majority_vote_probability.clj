...
(ns ^{:author "John Alan McDonald" :date "2016-12-22"
      :doc "Spikes probability forest example." }
  ...
  (:require [clojure.pprint :as pp]
            ...
            [taiga.test.classify.data.kolor :as kolor]
            [taiga.test.classify.data.record :as record]
            [taiga.test.classify.data.defs :as defs])
  (:import [taiga.test.java.data Kolor]))
;; mvn -Dtest=taiga.test.classify.spikes.majority-vote-probability clojure:test > tests.txt
;;------------------------------------------------------------------------------
(def ^:private prng0 
  (z/mersenne-twister-generator "17E2AC8550CB2B2E345D3BD26C938137"))
(def ^:private prng1 
  (z/mersenne-twister-generator "86E21A9AD0BD342B282719BB2C24DA95"))
(def nss (str *ns*))
(test/deftest spikes-majority-vote-probability
  (z/seconds 
    nss
    ...
    (let [options (defs/options (record/make-spikes-function 1.0))
      ...
      (test/is (== (float train-confusion-rate) (float 0.045654296875)))
      (test/is (= [31265 1496 0 7] train-confusion))
      (test/is (== (z/count (:data options)) (reduce + train-confusion)))
      (println "test:") (println test-confusion-rate) (println test-confusion)
      (test/is (== (float test-confusion-rate) (float 0.04803466796875)))
      (test/is (= [31185 1573 1 9] test-confusion))
      ...
      (test/is (== (float train-mad) (float 0.033237358500346405)))
      (println "test" test-mad)
      (test/is (== (float test-mad) (float 0.03400275117836277)))
      (println "train importance:")
      (pp/pprint (into (sorted-map)
                       (taiga/permutation-statistics 
                         metrics forest attributes (:data options) prng0)))
      (println "test importance:")
      (pp/pprint (into (sorted-map)
                       (taiga/permutation-statistics 
                         metrics forest attributes (:test-data options) prng1)))
