...
;; mvn -Dtest=taiga.test.classify.pyramid.majority-vote-probability clojure:test > tests.txt
;;------------------------------------------------------------------------------
(def nss (str *ns*))
(test/deftest pyramid-majority-vote-probability
  (z/seconds 
    nss
    ...
    (let [options (defs/options (record/make-pyramid-function 1.0))
      ...
      (test/is (== (float train-confusion-rate) (float 0.248260498046875)))
      (test/is (= [12118 3989 4146 12515] train-confusion))
      (test/is (== (z/count (:test-data options)) (reduce + test-confusion)))
      (test/is (== (float test-confusion-rate) (float 0.250823974609375)))
