...
;; mvn -Dtest=taiga.test.classify.pyramid.bias-positive-fraction-probability clojure:test > tests.txt
;;------------------------------------------------------------------------------
(def nss (str *ns*))
(test/deftest pyramid-bias-positive-fraction-probability
  (z/seconds 
    nss
    ...
    (let [options (defs/options (record/make-pyramid-function 0.1))
      ...
      (test/is (== (float train-mad) (float 0.011676578848431641)))
      (println "test" test-mad)
      (test/is (== (float test-mad) (float 0.011310470580056619)))
